...
export interface JobQuestionBank {
  technical: InterviewQuestion[];
  aptitude: InterviewQuestion[];
}

export const questionBank: Record<string, JobQuestionBank> = {
...
export function getQuestionsForJob(jobTitle: string, type: "technical" | "aptitude"): InterviewQuestion[] {
  // Try to match specific jobs, fallback to Full Stack if not found exactly
  let matchedBank = questionBank["Senior Full Stack Engineer"]; 
  
  if (jobTitle.includes("Frontend")) {
    matchedBank = questionBank["Frontend Developer"];
  } else if (jobTitle.includes("Backend")) {
    matchedBank = questionBank["Backend Systems Engineer"];
  }
  
  return matchedBank[type];
}
