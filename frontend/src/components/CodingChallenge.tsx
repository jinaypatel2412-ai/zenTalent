...
export function CodingChallenge({ jobTitle, onComplete }: CodingChallengeProps) {
  ...
  const [evaluations, setEvaluations] = useState<any[]>(new Array(CODING_PROBLEMS.length).fill(null));
  ...
  const handleLanguageChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    const newCodes = codes.map((c, i) => {
      // If code is still the starter code of the old language, switch it to new language starter code
      // Otherwise keep it
  ...
  const evaluateCode = async () => {
    ...
    try {
      // Mock evaluation response since Supabase edge function is not setup locally
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        {currentProblem === CODING_PROBLEMS.length - 1 ? (
          <button onClick={handleFinish} disabled={!allEvaluated}
            className="px-6 py-2.5 rounded-xl bg-primary text-primary-foreground text-sm font-semibold hover:bg-primary/90 transition-colors shadow-sm disabled:opacity-50 flex items-center gap-2">
